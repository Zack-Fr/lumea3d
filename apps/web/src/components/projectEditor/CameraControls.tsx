...
const CameraControlsComponent: React.FC<CameraControlsProps> = ({ 
  ...
  // Safety check - don't render controls if camera or gl are not ready
  if (!camera || !camera.position || !gl || !gl.domElement) {
    console.warn('CameraControls: Camera or GL context not ready, skipping render');
    return null;
  }
  ...
  if (cameraMode === 'fps') {
    return (
      <>
        {/* Orbit controls for when WASD is not active - only render if camera and gl are ready */}
        {camera && camera.position && gl && gl.domElement && (
          <OrbitControls
            key={`fps-controls-${cameraMode}-${isWASDActive}`}
  ...
  // Orbit mode (default) - only render if camera and gl are ready
  return (
    <>
      {camera && camera.position && gl && gl.domElement && (
        <OrbitControls
          key={`orbit-controls-${cameraMode}`}
          ref={orbitControlsRef}
          args={[camera, gl.domElement]}
          enablePan={enablePan}
          enableZoom={enableZoom}
